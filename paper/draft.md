# Cheaper Models, Honest Intervals: An Evaluation Protocol for Multi-View Molecular Property Prediction
...
**Status: drafted, not yet verified.** Every entry below must be opened at the publisher and
checked — year, venue, volume, pages, DOI — before submission. Entries marked ⚠ are the ones
we are least confident about and must not be pasted into a reference list as they stand.
`paper/references_gap.md` records which claim each reference supports and what is still
missing.
...
3. Mobley, D. L., & Guthrie, J. P. (2014). FreeSolv: a database of experimental and
   calculated hydration free energies. *Journal of Computer-Aided Molecular Design*, 28(7),
   711–720. ⚠
...
9. Xiong, Z., Wang, D., Liu, X., Zhong, F., Wan, X., Li, X., Li, Z., Luo, X., Chen, K.,
   Jiang, H., & Zheng, M. (2020). Pushing the boundaries of molecular representation for
   drug discovery with the graph attention mechanism. *Journal of Medicinal Chemistry*,
   63(16), 8749–8760. — AttentiveFP, external baseline (§5.5). ⚠
10. Yang, K., Swanson, K., Jin, W., Coley, C., Eiden, P., Gao, H., Guzman-Perez, A.,
    Hopper, T., Kelley, B., Mathea, M., et al. (2019). Analyzing learned molecular
    representations for property prediction. *Journal of Chemical Information and Modeling*,
    59(8), 3370–3388. — Chemprop / D-MPNN, external baseline (§5.5). ⚠
...
### The fusion mechanisms this paper tests

14. The cross-attention multi-view proposal implemented as the `xattn` rung (MvMRL, held in
    the project reference collection). Full citation to be completed before submission. ⚠
15. The Kronecker / second-order fusion proposal implemented as the `bilinear` rung (KROVEX).
    Full citation to be completed. ⚠ Its own ablation reports Kronecker fusion beating plain
    concatenation on FreeSolv and ESOL; §5.3 runs that comparison on eight datasets with
    multiplicity correction and finds it unit-dependent.
...
20. Angelopoulos, A. N., & Bates, S. (2023). Conformal prediction: a gentle introduction.
    *Foundations and Trends in Machine Learning*, 16(4), 494–591. ⚠ — §6's entry point.
21. Sadinle, M., Lei, J., & Wasserman, L. (2019). Least ambiguous set-valued classifiers with
...
25. Guo, C., Pleiss, G., Sun, Y., & Weinberger, K. Q. (2017). On calibration of modern neural
    networks. *ICML*. — temperature scaling, proved inert for binary conformal in §6.3.
26. Tursunbadalov & Tursunbadalov (2026). A quiet failure in calibrated virtual screening:
    marginal conformal prediction under-covers the minority class, and a class-conditional fix
    recovers it. *arXiv:2607.06605*. ⚠ — **independently reports the failure §6.2 measures,
    four months before this work.** §6.2 is written as corroboration on a wider model set, not
    as discovery.
...
27. van Tilborg, D., Alenicheva, A., & Grisoni, F. (2022). Exposing the limitations of
    ...
    Modeling*, 62(23), 5938–5951. (See also the published correction.) — §6.7's direct
    predecessor; the definitional difference is recorded there.
...
python -m scripts.check_paper       # 72 assertions: the prose matches the archives
