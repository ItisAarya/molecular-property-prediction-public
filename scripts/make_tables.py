...
def main_table():
    ...
    for tag, label in TAGS:
        ...
        for ds in DATASETS:
            ...
            m, _, h, n_seeds = ci95([value(v, ds, tag) for v in seeds])
            if dc is None and n_seeds == 0:
                row[ds] = "--"
                continue
            any_value = True
            dc_txt = f"{dc:.4f}" if dc is not None else "--"
            seed_txt = ("--" if n_seeds == 0 else
                        f"{m:.4f}" if n_seeds == 1 else f"{m:.4f}+/-{h:.4f}")
...
def main():
    ...
    combined = ["# Results\n\nGenerated by `scripts/make_tables.py` from "
                "`results/runs/`. Do not edit by hand.\n"]
